...
describe("listSavedFiltersForUser", () => {
  it("splits the user's filters into recent and saved groups", async () => {
    await upsertAppliedFilter({ userId: "u1", scope: "tasks", criteria: EMPTY_TASK_FILTER_CRITERIA, saved: false, label: null });
    ...
    const groups = await listSavedFiltersForUser("u1", "tasks");
    expect(groups.recent).toHaveLength(1);
    expect(groups.saved).toHaveLength(1);
  });

  it("never includes another user's filters", async () => {
    await upsertAppliedFilter({ userId: "u2", scope: "tasks", criteria: EMPTY_TASK_FILTER_CRITERIA, saved: false, label: null });
  ...
  it("excludes a legacy-shaped record from both recent and saved without throwing", async () => {
    (await getDb()).savedFilters.legacy = {
      id: "legacy",
      userId: "u1",
      scope: "tasks",

...
describe("saveFilterForUser", () => {
  it("records the filter as saved with the given label", async () => {
    const result = await saveFilterForUser("u1", "tasks", { ...EMPTY_TASK_FILTER_CRITERIA, search: "deploy" }, "My deploys");

    expect(parseSavedFilterQuery(result)).toMatchObject({ search: "deploy", saved: true, label: "My deploys" });
  });

  it("records a saved list filter under the lists scope", async () => {
    const result = await saveFilterForUser("u1", "lists", { ...EMPTY_LIST_FILTER_CRITERIA, search: "sprint" }, "My lists");

...
const FORBIDDEN_MESSAGE = "У вас нет прав на удаление этого списка";
...
export interface UseDeleteListResult {
  deleteList: (listId: string) => Promise<TaskList | null>;
  isPending: boolean;
  error: string | null;
}

/**
 * Client-side wrapper around `DELETE /api/lists/:id`. Owns request state
 * (pending/error) only — permission checks and soft-delete stay on the
...
export function useDeleteList(): UseDeleteListResult {
  ...
  const deleteList = useCallback(async (listId: string): Promise<TaskList | null> => {
  ...
  return { deleteList, isPending, error };
}
